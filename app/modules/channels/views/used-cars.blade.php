...
@foreach($cars as $car)
    <li data-model-id="{{ $car->product_id }}" data-model-year="{{ (int)$car->year }}" class="sub-offers-li">
        @if(!empty($car->images) && File::exists(public_path('uploads/galleries/thumbs/'.$car->images->name)))
        <div class="sub-offers-li-head" style="background: url({{ asset('uploads/galleries/thumbs/'.$car->images->name) }}) no-repeat center center / cover;">
