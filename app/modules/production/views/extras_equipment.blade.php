...
<section class="accessories complectations inf-block">
    <header><h1>{{ $product->meta->first()->title }}: Дополнительное оборудование</h1></header>
    @if(!is_null($product->extras_equipment) && count($product->extras_equipment))
        <dl class="acc-dl">
            @foreach($product->extras_equipment as $accessories_category_title => $accessories )
